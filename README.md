...
The table below gives an overview of the parameters in the files for reproducing all figures and tables in the article and supplement. Figures for other parameter combinations are contained in the folder *additional_figures*. The default parameters in for the figures in the article are *correlation = "pos"*, *not_penalize_rmle = FALSE*, and *q = 4*. For other values of these parameters (*correlation = "negative_cor"*, *not_penalize_rmle = TRUE*, or *q = 8*), the corresponding parameter is appended to the file name.
